...
from lib.reports import DBReport
...
class Controller(object):
    ...
    def setupReports(self, requester):
            ...
            if FileUtils.canWrite(directoryPath):
                ...
                if self.arguments.autoSaveFormat == 'simple':
                    report = SimpleReport(requester.host, requester.port, requester.protocol, requester.basePath,
                                          outputFile)
                if self.arguments.autoSaveFormat == 'db':
                    report = DBReport(requester.host, requester.port, requester.protocol, requester.basePath,
                                          outputFile)                    
        ...
        if self.arguments.DBOutputFile is not None:
            self.reportManager.addOutput(DBReport(requester.host, requester.port, requester.protocol,
                                                      requester.basePath, self.arguments.DBOutputFile))
